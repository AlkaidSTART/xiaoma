...
export default function McpConfigPage() {
  ...
  return (
    <main className="min-h-screen bg-[#F7F7F3] text-[#111111]">
      <div className="mx-auto flex min-h-screen w-full max-w-4xl flex-col px-5 py-5 md:px-7 md:py-6 lg:px-8">
        <header className="flex items-center justify-between border-b border-black/10 pb-4">
          ...
          <div className="text-right">
            ...
            <div className="text-sm text-black/55">紧凑模式</div>
          </div>
        </header>

        <section className="mt-5 rounded-[1.6rem] border border-black/10 bg-white p-5 shadow-[0_20px_50px_rgba(0,0,0,0.06)]">
          <div className="flex flex-wrap items-center justify-between gap-3 border-b border-black/10 pb-3">
            <div className="flex items-center gap-3">
              <div className={`h-2.5 w-2.5 rounded-full ${configured ? 'bg-emerald-500 shadow-[0_0_0_4px_rgba(16,185,129,0.12)]' : 'bg-black/25'}`} />
              <span className="text-[10px] uppercase tracking-[0.2em] text-black/45">{configured ? '已配置' : '未配置'}</span>
              <div className="inline-flex items-center gap-2 rounded-full border border-black/10 px-2.5 py-1 text-[11px] text-black/55">
                <ShieldCheck className="h-3.5 w-3.5" /> 自动保存
              </div>
            </div>
            <div className="text-right text-sm text-black/50">{server.title}</div>
          </div>
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-3">
            {MCP_SERVER_DEFINITIONS.map((item) => (
              <button
                ...
                className={`flex items-center justify-between rounded-xl border px-3 py-2 text-left text-xs transition-colors ${item.key === server.key ? 'border-black/10 bg-black text-white' : 'border-black/10 bg-white text-black/70 hover:bg-black/[0.03]'}`}
              >
                <span>{item.title}</span>
                <span className={`rounded-full px-1.5 py-0.5 text-[10px] uppercase tracking-[0.12em] ${item.key === server.key ? 'bg-white/10 text-white/75' : 'bg-black/[0.04] text-black/35'}`}>
          ...
          <div className="mt-4 rounded-xl border border-black/10 bg-black/[0.02] p-3 text-xs text-black/60">
            修改会自动保存到 IndexedDB，并在聊天时同步给后端上下文。
          </div>

          <div className="mt-4 grid gap-3 md:grid-cols-[auto,1fr] md:items-center">
            <label className="text-sm font-medium">启用 MCP</label>
            <label className="flex items-center justify-between rounded-xl border border-black/10 px-3 py-2">
              <span className="text-xs text-black/45">开启后会在聊天上下文中读取该配置</span>
              <input
                type="checkbox"
            ...
            <label className="self-start pt-2 text-sm font-medium">参数</label>
            <textarea
              rows={4}
              value={config.params}
          ...
          <div className="mt-4 flex items-center justify-between border-t border-black/10 pt-3 text-xs text-black/45">
